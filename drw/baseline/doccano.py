...
if __name__ == "__main__":
    ...
    parser.add_argument("--doccano_file", default="./input/train/ner", type=str, help="The doccano file exported from doccano platform.")
    ...
    parser.add_argument("--splits", default=[0.8, 0.2, 0], type=float, nargs="*", help="The ratio of samples in datasets. [0.6, 0.2, 0.2] means 60% samples used for training, 20% for evaluation and 20% for test.")
    ...
    parser.add_argument("--options", default=["超速", "正常"], type=str, nargs="+", help="Used only for the classification task, the options for classification")
    parser.add_argument("--prompt_prefix", default="被告人违规行为", type=str, help="Used only for the classification task, the prompt prefix for classification")
    args = parser.parse_args()
    do_convert()
    

    # 案件要素二分类
    filenames = os.listdir('./input/train/案件要素')
    for filename in filenames:
        promat = filename
        doccano_file = os.path.join('./input/train/案件要素', filename)
        print(doccano_file)
        options = ['正','负']
        parser = argparse.ArgumentParser()
        parser.add_argument("--doccano_file", default=doccano_file, type=str, help="The doccano file exported from doccano platform.")
        parser.add_argument("--save_dir", default="./data", type=str, help="The path of data that you wanna save.")
        parser.add_argument("--options", default=options, type=str, nargs="+", help="Used only for the classification task, the options for classification")
        parser.add_argument("--prompt_prefix", default=promat, type=str, help="Used only for the classification task, the prompt prefix for classification")   # config[filename]['prompt']
        parser.add_argument("--task_type", choices=['ext', 'cls'], default="cls", type=str, help="Select task type, ext for the extraction task and cls for the classification task, defaults to ext.")    
        parser.add_argument("--negative_ratio", default=0, type=int, help="Used only for the extraction task, the ratio of positive and negative samples, number of negtive samples = negative_ratio * number of positive samples")
        parser.add_argument("--splits", default=[0.8, 0.2, 0], type=float, nargs="*", help="The ratio of samples in datasets. [0.6, 0.2, 0.2] means 60% samples used for training, 20% for evaluation and 20% for test.")
        parser.add_argument("--is_shuffle", default=True, type=bool, help="Whether to shuffle the labeled dataset, defaults to True.")
        parser.add_argument("--seed", type=int, default=1000, help="Random seed for initialization")
        parser.add_argument("--separator", type=str, default='##', help="Used only for entity/aspect-level classification task, separator for entity label and classification label")
    ...
    # 刑档多分类
    promat = '刑档'
    doccano_file = os.path.join('./input/train/刑档')
    print(doccano_file)
    options = ['一档','二档','三档']
    parser = argparse.ArgumentParser()
    parser.add_argument("--doccano_file", default=doccano_file, type=str, help="The doccano file exported from doccano platform.")
